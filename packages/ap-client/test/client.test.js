...
test('publish() writes to the local store immediately, even while disconnected', async () => {
  ...
  const doc = await client.publish({ type: 'Note', fields: { content: 'hi' } });
...
test('publish() sets `to` from visibility', async () => {
  const { client } = makeClient();
  const doc = await client.publish({ type: 'Note', visibility: 'https://relay.example/followers' });
  assert.deepEqual(doc.to, ['https://relay.example/followers']);
});

test('an explicit id is honored instead of minting one', async () => {
  const { client } = makeClient();
  const doc = await client.publish({ id: 'https://relay.example/objects/fixed', type: 'Note' });
...
test('a persistent publish while disconnected queues in the outbox, not sent', async () => {
  ...
  await client.publish({ type: 'Note', collections: [OUTBOX] });
...
test('repeated offline edits to the same id collapse into a single outbox entry', async () => {
  const { client } = makeClient();
  await client.publish({ id: 'https://relay.example/objects/x', type: 'Note', fields: { content: 'v1' } });
  await client.publish({ id: 'https://relay.example/objects/x', type: 'Note', fields: { content: 'v2' } });
  assert.equal(await client.outboxSize(), 1);
});
...
test('ephemeral publish never queues, and is dropped silently if disconnected', async () => {
  ...
  await assert.doesNotReject(() =>
    client.publish({ type: 'qu:Signal', durability: DURABILITY.EPHEMERAL, fields: { kind: 'offer' } }),
  );
  assert.equal(await client.outboxSize(), 0);
  assert.deepEqual(transport.sent, []);
});
...
test('ephemeral publish sends live when connected', async () => {
  ...
  await client.publish({ type: 'qu:Signal', durability: DURABILITY.EPHEMERAL });
...
test('session publish never queues either', async () => {
  const { client } = makeClient();
  await client.publish({ type: 'qu:Cursor', durability: DURABILITY.SESSION, sessionId: 's1' });
  assert.equal(await client.outboxSize(), 0);
});
...
test('watch() reflects live local-store changes (integration with @qu/reactive)', async () => {
  ...
  await client.publish({ id: 'https://relay.example/objects/x', type: 'Note' });
  await new Promise((r) => setImmediate(r));
...
test('a mid-flush send failure leaves the remaining entries queued for next time', async () => {
  ...
  await client.publish({ id: 'https://relay.example/objects/1', type: 'Note', collections: [OUTBOX] });
  await client.publish({ id: 'https://relay.example/objects/2', type: 'Note', collections: [OUTBOX] });
